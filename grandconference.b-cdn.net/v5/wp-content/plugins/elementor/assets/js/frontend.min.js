...
(self.webpackChunkelementor = self.webpackChunkelementor || []).push([
  [819],
  {
    ...
    // Tentukan apakah arahnya Slide Right (sebaliknya)
    // Asumsi: Default adalah Slide Left (gerak ke kiri), jika setting 'slide_right' maka reverse.
    const isSlideRight = "slide_right" === e.background_slideshow_slide_transition;
    ...
    const t = {
        ...
        autoplay: {
            ...
            disableOnInteraction: false, // Tambahan: Agar autoplay tidak mati saat disentuh
            reverseDirection: isSlideRight // Gunakan fitur native Swiper untuk arah
        },
    ...
    // --- OPTIMASI ANTI LAG (PENTING) ---
    if ("yes" === e.background_slideshow_loop) {
        // ... di dalam getSwiperOptions ...

// Hitung jumlah gambar asli yang diupload user
const slidesCount = e.background_slideshow_gallery ? e.background_slideshow_gallery.length : 0;

if ("yes" === e.background_slideshow_loop) {
    // LOGIKA ADAPTIF (SMART LOGIC):
    // Jika gambar sedikit (<= 5), biarkan Swiper mengatur buffer sesuai jumlah gambar (aman & mulus).
    // Jika gambar BANYAK (> 5), kita paksa buffernya cuma 3 supaya browser tidak berat/lag.
    
    if (slidesCount > 5) {
        t.loopedSlides = 3; 
    } else {
        t.loopedSlides = slidesCount; // Buffer sesuai jumlah gambar (misal 5 gambar = 5 buffer)
    }
}
// ...
    }

    switch (e.background_slideshow_slide_transition) {
        case "fade":
            t.effect = "fade";
            t.fadeEffect = { crossFade: true };
            break;
        case "slide_down":
            t.autoplay.reverseDirection = true;
            t.direction = "vertical";
            break;
        case "slide_up":
            t.direction = "vertical";
            break;
        // Case slide_left dan slide_right ditangani oleh default horizontal + autoplay.reverseDirection di atas
    }

    if ("yes" === e.background_slideshow_lazyload) {
        t.lazy = {
            loadPrevNext: true,
            loadPrevNextAmount: 2, // Naikkan jadi 2 agar gambar berikutnya sudah siap lebih awal
    ...
    // Hapus logika direction "rtl" dari sini. Biarkan default "ltr".
    // Arah gerak diatur oleh 'autoplay.reverseDirection' di getSwiperOptions.
    const $container = jQuery("<div>", { class: classes.swiperContainer, dir: "ltr" }),
          $wrapper = jQuery("<div>", { class: classes.swiperWrapper }),
          isKenBurns = settings.background_slideshow_ken_burns,
          isLazyLoad = "yes" === settings.background_slideshow_lazyload;

    let backgroundClasses = classes.slideBackground;

    if (isKenBurns) {
        backgroundClasses += ` ${classes.kenBurns}`;
        const zoomDir = "in" === settings.background_slideshow_ken_burns_zoom_direction ? "kenBurnsIn" : "kenBurnsOut";
        backgroundClasses += ` ${classes[zoomDir]}`;
    }

    if (isLazyLoad) {
        backgroundClasses += " swiper-lazy";
    }

    // Fragment document untuk performa insert DOM (Batch Insert)
    const docFrag = document.createDocumentFragment();

    settings.background_slideshow_gallery.forEach((item) => {
        const $slide = jQuery("<div>", { class: classes.swiperSlide });
        let $bgElement;

        if (isLazyLoad) {
            const $preloader = jQuery("<div>", { class: classes.swiperPreloader });
            $bgElement = jQuery("<div>", {
                class: backgroundClasses,
                "data-background": item.url // Swiper akan mengambil ini saat lazy load aktif
            }).append($preloader);
        } else {
            $bgElement = jQuery("<div>", {
                class: backgroundClasses,
        ...
        $slide.append($bgElement);
        // Append ke fragment, bukan langsung ke wrapper jQuery berulang kali
        docFrag.appendChild($slide[0]);
    });

    // Masukkan semua slide sekaligus ke wrapper
    ...
    // Simpan referensi
    this.elements.$slides = $wrapper.children(); // Ambil langsung dari wrapper yg sudah jadi
